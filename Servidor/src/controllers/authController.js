...
const autenticarUsuario = async (req, res) => {
  ...
  try {
    ...
    if (!usuario) {
      ...
      if (!usuario) {
        return res.status(400).json({ msg: "No hay usuarios regitrados con ese número de identificación" });
      }
      tipe = "profesional";
    }
    ...
    jwt.sign(
      ...
      (error, token) => {
        ...
        return res.status(200).json({ msg: 'Sesión iniciada', tipo: tipe, token });
      }
    );
  } catch (error) {
...
const usuarioAutenticado = async (req, res) => {
  try {
    ...
    if (req.usuario.tipo == "profesional") {
      ...
      usuario = await Paciente.findOne({ _id: req.usuario._id });
      tipe = "paciente";
    }
    console.log(usuario)
